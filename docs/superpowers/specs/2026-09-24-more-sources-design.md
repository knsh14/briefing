...
- 既存の `--out-dir` の中身は実行開始時に消す。消すのは、ディレクトリが空か、中身がすべて `manifest.json` か `^\d{2}-[a-z0-9-]+\.txt$` に合うファイル（このスクリプト自身の出力）のときに限る。それ以外のファイルがあれば消さず、標準エラーにそのファイル名を出して終了コード 1 で止まる。パスの指定を誤って無関係なファイルを消さないためである。フィードごとのファイルは `manifest.json` より先に書くので、途中で止まった実行の残りは `manifest.json` がなくても消せる。

...
`<skill-dir>/repos.json` に監視対象リポジトリを記載する。
...
`<skill-dir>/repos.json` を読み込む。存在しない場合は、ユーザーに監視したいリポジトリを確認して作成する。
...
保存先: `github/YYYY-MM-DD.md`
- 同名ファイルが既にある場合は上書きする

### Step 5: 文章校正

保存した `github/YYYY-MM-DD.md` を、次の2段階で校正・修正する。
...
> Issue/PR タイトルや技術用語・識別子（関数名、API 名、ファイル名など）は原語のまま残す。校正対象は日本語の地の文（要約・傾向・選定理由）に限る。
...
- 校正結果（japanese-tech-writing の規範修正件数、textlint の自動修正件数 / 手動対応が必要な件数）

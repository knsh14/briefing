...
各フィードには任意で `include_links_from`（ページURLの配列）を指定できる。指定すると、それらのページからリンクを抽出し、記事のリンクがそのいずれかに一致するものだけを対象にする（フィード全体に混ざる無関係な記事を除外するため）。いずれかのページ取得に失敗した場合はそのフィード全体が失敗として扱われる。
